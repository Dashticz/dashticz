...
var DashticzWidgetEditor = (function () {
  ...
  function _backButtonHtml() {
    var backLabel =
      ...
      'Back';
    return (
      '<button type="button" class="btn btn-secondary we-back-btn">' +
  ...
  function _effectiveWidgetConfigIcon(item, options) {
    ...
    return (
      _fontIconClass($mount.find('.col-icon em, .sunrise-header em').first()) ||
      item.icon ||
      'fas fa-question'
    );
  }
  ...
  function _hydrateWidgetBlockOptions(item, definition) {
    ...
    if (
      item.id === 'calendar' &&
      !definition.icalurl &&
      Array.isArray(definition.calendars)
    ) {
  ...
  function _normaliseCalendarSources(icalurl, legacyCalendars) {
    ...
    if (typeof icalurl === 'string') {
      ...
      icalurl &&
      typeof icalurl === 'object' &&
      !Array.isArray(icalurl)
    ) {
  ...
  function openConfig(widgetId, options) {
    ...
    if (options.draft) {
      ...
      if (options.draft.blockOptions) {
        widgetBlockOptions[widgetId] = $.extend(
          true,
          {},
          options.draft.blockOptions
        );
      }
      ...
      column.blocks.forEach(function (reference) {
        ...
        if (item.id === 'weather') {
          if (typeof definition.showRain !== 'undefined') {
            widgetConfigs.weather.weather_show_rain = Number(
              definition.showRain
            )
              ? 1
              : 0;
          ...
          if (typeof definition.showDescription !== 'undefined') {
            widgetConfigs.weather.weather_show_description = Number(
          ...
          if (typeof definition.showWind !== 'undefined') {
            widgetConfigs.weather.weather_show_wind = Number(
              definition.showWind
            )
              ? 1
              : 0;
          ...
          if (typeof definition.showGust !== 'undefined') {
            widgetConfigs.weather.weather_show_gust = Number(
              definition.showGust
            )
              ? 1
              : 0;
        ...
        if (
          item.id === 'calendar' &&
          typeof definition.maxitems !== 'undefined'
        ) {
          widgetConfigs.calendar.calendar_maxitems = String(
            definition.maxitems
          );
        }
        if (item.id === 'garbage') {
          if (typeof definition.maxitems !== 'undefined') {
            widgetConfigs.garbage.garbage_maxitems = String(
              definition.maxitems
            );
          }
        ...
        if (
          ...
          if (typeof definition.showSeconds !== 'undefined') {
            widgetConfigs.clock.showSeconds = Number(definition.showSeconds)
              ? 1
              : 0;
          }
          if (
            typeof definition.clockFace !== 'undefined' &&
            definition.clockFace !== null
          ) {
        ...
        if (item.id === 'camera') {
          if (Array.isArray(definition.cameras) && definition.cameras.length) {
            widgetConfigs.camera.cameras = definition.cameras.map(
        ...
        if (item.id === 'iframe') {
          ...
          if (typeof definition.scrollbars !== 'undefined') {
            widgetConfigs.iframe.scrollbars =
              definition.scrollbars === false ? 0 : 1;
          }
  ...
  function _hydrateLogWidgetConfig(definition) {
    ...
    widgetConfigs.log.scrolltimeout =
      typeof definition.scrolltimeout !== 'undefined'
        ? String(definition.scrolltimeout)
        : '60';
    widgetConfigs.log.ascending = definition.ascending === false ? 0 : 1;
    widgetConfigs.log.maxitems =
      typeof definition.maxitems !== 'undefined'
        ? String(definition.maxitems)
        : '';
  }
  ...
  function _hydrateOwmWidgetConfig(definition) {
    ...
    widgetConfigs.owm.layout =
      typeof definition.layout !== 'undefined'
    ...
    _orderedColumnKeys().forEach(function (columnKey) {
      ...
      column.blocks.forEach(function (reference) {
        ...
        layoutOrder.push({
          ...
          width: Math.max(1, Math.min(12, parseInt(definition.width, 10) || 3)),
          height: parseInt(definition.height, 10) || null,
        });
      });
    });
  ...
  function _lmsWidgetCardHtml() {
    return (
      ...
      _t(
        'lms_description',
        'Now playing info for a Lyrion Music Server (Logitech Media Server) player.'
      ) +
      '</div></div>' +
      '<div class="we-widget-status">' +
  ...
  function _cfgField(key, label, type, value, opts, help) {
    ...
    if (type === 'text') {
      html +=
        '<input type="text" class="form-control form-control-sm we-widget-field" id="' +
        _esc(id) +
        '" data-cfg-key="' +
        _esc(key) +
        '" value="' +
      ...
      html +=
        '<select class="form-select form-select-sm we-widget-field" id="' +
        _esc(id) +
        '" data-cfg-key="' +
        _esc(key) +
        '">';
      ...
      html +=
        ...
        _esc(key) +
        '"' +
        (typeof opts.min !== 'undefined'
          ? ' min="' + _esc(opts.min) + '"'
          : '') +
        (typeof opts.max !== 'undefined'
          ? ' max="' + _esc(opts.max) + '"'
          : '') +
        (typeof opts.step !== 'undefined'
          ? ' step="' + _esc(opts.step) + '"'
          : '') +
        ' value="' +
        _esc(String(value !== null && value !== undefined ? value : '')) +
  ...
  function _customFieldRowHtml(row) {
    ...
    return (
      ...
      _esc(
        lowerField === 'image' ? 'custom/icon.png' : _t('setting', 'Setting')
      ) +
      '" value="' +
      _esc(row.setting || '') +
      '">' +
  ...
  function _buildConfigModalHtml(item) {
    ...
    if (item.id === 'weather') {
      ...
      fields +=
        ...
        (cfg.provider === 'openweather' ? ' selected' : '') +
        '>' +
        _t('openweather', 'OpenWeather') +
        '</option>' +
        '<option value="wunderground"' +
        (cfg.provider === 'wunderground' ? ' selected' : '') +
        '>' +
        _t('weather_underground', 'Weather Underground') +
        '</option>' +
        '</select></div>';
      fields +=
      ...
      fields += _cfgField(
        'owm_api',
        lw.owm_api || 'OpenWeather API key',
        'text',
        cfg.owm_api
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'owm_name',
        lw.owm_name || 'Display name',
        'text',
        cfg.owm_name
      );
      fields += _cfgField(
        'owm_country',
        lw.owm_country || 'Country code',
        'text',
        cfg.owm_country
      );
      fields += _cfgField(
        'owm_lang',
        lw.owm_lang || 'Language code',
        'text',
        cfg.owm_lang,
        null,
        lw.owm_lang_help || ''
      ...
      fields += _cfgField(
        'owm_min',
        lw.owm_min || 'Show minimum temperature',
        'checkbox',
        cfg.owm_min,
        null,
        lw.owm_min_help || ''
      ...
      fields += _cfgField(
        'weather_show_rain',
        lw.show_rain || 'Show rain',
        'checkbox',
        cfg.weather_show_rain
      );
      fields += _cfgField(
        'weather_show_description',
        lw.show_description || 'Show description',
        'checkbox',
        cfg.weather_show_description
      );
      fields += _cfgField(
        'weather_show_wind',
        lw.show_wind || 'Show wind',
        'checkbox',
        cfg.weather_show_wind
      );
      fields += _cfgField(
        'weather_show_gust',
        lw.show_gust || 'Show gusts',
        'checkbox',
        cfg.weather_show_gust
      );
      fields += _cfgField(
        'weather_icons',
        lw.icons || 'Weather icons',
        'select',
        cfg.weather_icons || 'line',
        iconOpts
      );
      fields += '</div>';
      fields +=
      ...
      fields += _cfgField(
        'wu_api',
        lw.wu_api || 'Weather Underground API key',
        'text',
        cfg.wu_api
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'wu_name',
        lw.wu_name || 'Display name (WU)',
        'text',
        cfg.wu_name
      );
      fields += _cfgField(
        'wu_country',
        lw.wu_country || 'Country code (WU)',
        'text',
        cfg.wu_country
      );
      fields += '</div>';
      fields += _cfgHeading(
        lw.shared_display || _t('general_display', 'General display')
      );
      fields += _cfgField(
        'use_fahrenheit',
        lw.use_fahrenheit || 'Use Fahrenheit',
        'checkbox',
        cfg.use_fahrenheit
      );
      fields += _cfgField(
        'use_beaufort',
        lw.use_beaufort || 'Use Beaufort',
        'checkbox',
        cfg.use_beaufort
      );
    } else if (item.id === 'calendar') {
      ...
      fields +=
        ...
        _clockOption(
          'basicclock',
          _t('basic_clock', 'Basic clock'),
          currentClockType
        ) +
        _clockOption(
          'stationclock',
          _t('station_clock', 'Station clock'),
          currentClockType
        ) +
        _clockOption(
          'flipclock',
          _t('flipclock', 'Flipclock'),
          currentClockType
        ) +
        _clockOption(
          'haymanclock',
          _t('hayman_clock', 'Hayman clock'),
          currentClockType
        ) +
        _clockOption(
          'miniclock',
          _t('miniclock', 'Miniclock'),
          currentClockType
        ) +
        '</select>' +
        '<img class="we-clock-preview" id="we-cfg-clock-preview" src="' +
        _clockPreviewSrc(currentClockType) +
        '" alt="">' +
        '</div>';
      ...
      fields += _cfgField(
        ...
        _t('clock_dial', 'Dial'),
        'select',
        ccfg.dial || 'GermanStrokeDial',
        dialOpts
      );
      fields += _cfgField(
        'hourhand',
        _t('clock_hour_hand', 'Hour hand'),
        'select',
        ccfg.hourhand || 'PointedHourHand',
        hourOpts
      );
      fields += _cfgField(
        'minutehand',
        _t('clock_minute_hand', 'Minute hand'),
        'select',
        ccfg.minutehand || 'PointedMinuteHand',
        minuteOpts
      );
      fields += _cfgField(
        'secondhand',
        _t('clock_second_hand', 'Second hand'),
        'select',
        ccfg.secondhand || 'HoleShapedSecondHand',
        secondOpts
      );
      fields += _cfgField(
        'boss',
      ...
      fields += _cfgField(
        'minutehandbehavior',
        _t('clock_minute_behavior', 'Minute-hand behavior'),
        'select',
        ccfg.minutehandbehavior || 'BouncingMinuteHand',
        minuteBehOpts
      );
      fields += _cfgField(
        'secondhandbehavior',
        _t('clock_second_behavior', 'Second-hand behavior'),
        'select',
        ccfg.secondhandbehavior || 'OverhastySecondHand',
        secondBehOpts
      );
      fields += '</div>';
      ...
      fields += _cfgField(
        ...
        'text',
        gcfg.garbage_zipcode
      );
      fields += _cfgField(
        'garbage_street',
        lg.garbage_street || 'Street',
        'text',
        gcfg.garbage_street
      );
      fields += _cfgField(
        'garbage_housenumber',
        lg.garbage_housenumber || 'House number',
        'text',
        gcfg.garbage_housenumber
      );
      fields += _cfgField(
        'garbage_housenumberadd',
        lg.garbage_housenumberaddition || 'House-number addition',
        'text',
        gcfg.garbage_housenumberadd
      );
      fields += _cfgField(
        'garbage_maxitems',
      ...
      fields += _cfgField(
        ...
        'text',
        gcfg.garbage_icalurl
      );
      fields += _cfgField(
        'google_api_key',
        lg.google_api_key || 'Google API key',
        'text',
        gcfg.google_api_key
      );
      fields += _cfgField(
        'garbage_calendar_id',
        lg.garbage_calendar_id || 'Google Calendar ID',
        'text',
        gcfg.garbage_calendar_id,
        null,
      ...
      fields += _cfgField(
        ...
        'checkbox',
        gcfg.garbage_hideicon
      );
      fields += _cfgField(
        'garbage_icon_use_colors',
        lg.garbage_icon_use_colors || 'Use icon colors',
        'checkbox',
      ...
      fields += _cfgField(
        'sonarr_url',
        lm.sonarr_url || 'Sonarr URL',
        'text',
        scfg.sonarr_url
      );
      fields += _cfgField(
        'sonarr_apikey',
        lm.sonarr_apikey || 'Sonarr API key',
        'text',
        scfg.sonarr_apikey
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'spot_clientid',
        lm.spot_clientid || 'Spotify Client ID',
        'text',
        spcfg.spot_clientid
      );
    } else if (item.id === 'publictransport') {
      ...
      fields +=
        ...
        _ptOption(
          'treinen',
          _t('trains_nl', 'Trains (NL)'),
          ptcfg.provider || 'treinen'
        ) +
      ...
      fields += _cfgField(
        'waqi_city',
        lw.waqi_city || 'WAQI city code',
        'text',
        lcfg.waqi_city,
        null,
        lw.waqi_city_help || 'Find your city code on aqicn.org.'
      );
      fields += _cfgField(
        'waqi_layout',
        lw.waqi_layout || 'Layout',
        'select',
        lcfg.waqi_layout || 'large',
        {
          xsmall: 'Extra small',
          small: 'Small',
          large: 'Large',
      ...
      fields += _cfgField(
        'default_news_url',
        lg2.default_news_url || 'News URL',
        'text',
        ncfg.default_news_url
      );
      fields += _cfgField(
        'news_scroll_after',
      ...
      fields += _cfgField(
        'iframe_height',
        li.iframe_height || 'Height (px)',
        'text',
        icfg.height,
        null,
        li.iframe_height_help ||
          'Optional legacy fixed height. Leave empty when using aspect ratio.'
      );
      fields += _cfgField(
        'iframe_scrollbars',
        li.iframe_scrollbars || 'Show scrollbars',
        'checkbox',
        icfg.scrollbars
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'iframe_forcerefresh',
        li.iframe_forcerefresh || 'Force cache refresh',
        'checkbox',
        icfg.forcerefresh
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'xmltv_channels',
        lx.xmltv_channels || 'Channels (comma-separated)',
        'text',
        xcfg.channels,
        null,
        lx.xmltv_channels_help ||
      ...
      fields += _cfgField(
        'scrolltimeout',
        llog.log_scrolltimeout || 'Auto-scroll resumes after (seconds)',
        'number',
        logcfg.scrolltimeout,
        { min: 0, max: 3600, step: 1 },
        llog.log_scrolltimeout_help ||
          'Delay after manual scrolling until auto-scroll is activated again. Default: 60.'
      );
      fields += _cfgField(
        'ascending',
        llog.log_ascending || 'Newest log lines at the bottom',
        'checkbox',
        logcfg.ascending
      );
      fields += _cfgField(
      ...
      fields += _cfgField(
        'height',
        llog.log_height || 'Height (px)',
        'text',
        logcfg.height,
        null,
        llog.log_height_help ||
          'Optional fixed height. Leave empty to use the automatic height.'
      );
      fields += _cfgField(
        'aspectratio',
      ...
      for (var layoutNr = 1; layoutNr <= 24; layoutNr++) {
        owmLayoutOpts[layoutNr] =
          (lowm.owm_layout_option || 'Layout') + ' ' + layoutNr;
      }
      fields += _cfgField(
        'apikey',
        lowm.owm_apikey || 'API key',
        'text',
        owmcfg.apikey,
        null,
        lowm.owm_apikey_help ||
          "Leave empty to use the global config['owm_api'] setting (configured on the Weather widget)."
      );
      fields += _cfgField(
        'layout',
        lowm.owm_layout || 'Layout',
        'select',
        owmcfg.layout || '11',
        owmLayoutOpts
      );
      fields += _cfgField(
        'city',
        lowm.owm_city_field || 'City',
        'text',
        owmcfg.city,
        null,
        lowm.owm_city_field_help ||
          "City name or OpenWeatherMap city id. Leave empty to use the global config['owm_city'] setting."
      );
      fields += _cfgField(
        'country',
        lowm.owm_country_field || 'Country',
        'text',
        owmcfg.country,
        null,
        lowm.owm_country_field_help ||
          "Country code, e.g. 'nl'. Leave empty to use the global config['owm_country'] setting."
      );
    } else if (item.id === 'timegraph') {
      ...
      fields += _cfgField(
        'duration',
        ltg.timegraph_duration || 'Duration (seconds)',
        'number',
        tgcfg.duration,
        { min: 1, max: 86400, step: 1 },
        ltg.timegraph_duration_help ||
          'Duration of the moving chart window in seconds. Default: 300.'
      );
      fields += _cfgField(
        'height',
        ltg.timegraph_height || 'Height (px)',
        'text',
        tgcfg.height,
        null,
        ltg.timegraph_height_help ||
          'Optional fixed height, e.g. 300px. Leave empty to use the automatic height.'
      );
      fields += _cfgHeading(ltg.timegraph_axes || 'Axes');
      fields += _cfgField(
        'xTicks',
        ltg.timegraph_xticks || 'X-axis labels (count)',
        'number',
        tgcfg.xTicks,
        { min: 1, max: 100, step: 1 },
        ltg.timegraph_xticks_help ||
          'Number of labels on the time axis. Default: 10.'
      );
      fields += _cfgField(
        'yTicks',
        ltg.timegraph_yticks || 'Y-axis labels (count)',
        'number',
        tgcfg.yTicks,
        { min: 1, max: 100, step: 1 },
        ltg.timegraph_yticks_help ||
          'Number of labels on the vertical axis. Default: 5.'
      );
      fields += _cfgField(
        'xLabels',
        ltg.timegraph_xlabels || 'Show time-axis labels',
        'checkbox',
        tgcfg.xLabels
      );
      ...
      fields += _cfgField(
        'animation',
        ltg.timegraph_animation || 'Animation (ms)',
        'number',
        tgcfg.animation,
        { min: 0, max: 10000, step: 1 },
        ltg.timegraph_animation_help ||
          'Duration of the animation effect in milliseconds. Default: 0.'
      );
      fields += _cfgField(
        'lineTension',
        ltg.timegraph_linetension || 'Line tension',
        'number',
        tgcfg.lineTension,
        { min: 0, max: 1, step: 0.1 },
        ltg.timegraph_linetension_help ||
          'Smooths the graph line. 0 = no smoothing, default 0.1.'
      );
      fields += _cfgField(
        'pointRadius',
        ltg.timegraph_pointradius || 'Point radius',
        'number',
        tgcfg.pointRadius,
        { min: 0, max: 20, step: 1 },
        ltg.timegraph_pointradius_help ||
          'Size of the dot for each data sample. Default: 1.'
      );
      fields += _cfgHeading(ltg.timegraph_values || 'Values');
      fields +=
        '<p class="form-text">' +
        _esc(
          ltg.timegraph_values_help ||
            'Add one value per device field to show. Set IDX per value to combine data from several devices in one graph.'
        ) +
        '</p>';
      fields += '<div id="we-cfg-timegraph-list">';
      (tgcfg.values && tgcfg.values.length
        ? tgcfg.values
        : [_defaultTimegraphValueRow()]
  ...
  function _openConfigModal(widgetId, callbacks) {
    ...
    function refreshCustomFieldButtons() {
      ...
      $cfgModal.find('.we-custom-field-remove').each(function () {
        var isSystem = $(this)
          .closest('.we-custom-field-row')
          .hasClass('we-system-field-row');
    ...
    function openCustomImagePicker($row) {
      ...
      var selectedPath = String(
        $row.find('.we-custom-field-setting').val() || ''
      );
      closeCustomImagePickers();
      ...
      $picker
        .find('.dt-custom-image-status')
        .show()
        .text(_t('loading_images', 'Loading images…'));
      ...
      _loadCustomImages()
        ...
        .fail(function () {
          ...
          $picker
            .find('.dt-custom-image-status')
            .show()
            .text(_t('custom_images_error', 'Unable to load custom images.'));
        });
    }
    ...
    $cfgModal.on(
      'click focus',
      '.we-icon-field-row .we-custom-field-setting',
      function () {
    ...
    $cfgModal.on('click', function (event) {
      if (
        $(event.target).closest(
          '.dt-custom-image-picker, .we-custom-field-setting'
        ).length
      )
        return;
      closeCustomImagePickers();
    ...
    $cfgModal.on('click', '#we-calendar-add', function () {
      ...
      $cfgModal
        .find('.we-calendar-remove')
        .prop('disabled', $cfgModal.find('.we-calendar-row').length <= 1);
    });
    ...
    $cfgModal.on('click', '#we-camera-add', function () {
      ...
      $('#we-cfg-camera-list').append(
        _cameraRowHtml(_defaultCameraConfig(index), index)
      );
      $cfgModal
        .find('.we-camera-remove')
        .prop('disabled', $cfgModal.find('.we-camera-row').length <= 1);
    });
    ...
    function _renumberRadioRows() {
      ...
      $cfgModal
        .find('.we-radio-remove')
        .prop('disabled', $cfgModal.find('.we-radio-row').length <= 1);
    }
    ...
    function _renumberTimegraphValueRows() {
      ...
      $cfgModal
        .find('.we-timegraph-value-remove')
        .prop(
          'disabled',
    ...
    $cfgModal.on('click', '#we-cfg-ok-btn', function () {
      var valid = true;
      var existingBlockOptions =
        widgetBlockOptions[widgetId] || _defaultWidgetBlockOptions(item);
      var pendingTitle = '';
      ...
      var pendingBlockOptions = {
        ...
        preservedFields: $.extend(
          {},
          existingBlockOptions.preservedFields || {}
        ),
      };
      var customKeys = {};
      $cfgModal.find('.we-custom-field-row').each(function () {
        if (!valid) return;
        var rawField = $.trim(
          $(this).find('.we-custom-field-name').val() || ''
        );
        var rawSetting = $.trim(
          $(this).find('.we-custom-field-setting').val() || ''
        );
        ...
        if (!field || !/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(field)) {
          ...
          $('.we-cfg-message')
            ...
            .text(_t('invalid_field', 'Enter a valid Field and Setting.'));
        ...
        if (lowerField === 'icon' || lowerField === 'image') {
          if (!pendingBlockOptions.icon) {
            ...
            $('.we-cfg-message')
              ...
              .text(
                _t(
                  'icon_requires_checkbox',
                  'Enable Icon before using the icon field.'
                )
              );
            $(this).find('.we-custom-field-name').trigger('focus');
            return;
        ...
        if (!rawSetting || _isProtectedCustomWidgetProperty(lowerField)) {
          ...
          $(this)
            .find(
              !rawSetting ? '.we-custom-field-setting' : '.we-custom-field-name'
            )
            .trigger('focus');
          return;
        }
        ...
        if (!parsedSetting.valid) {
          ...
          $('.we-cfg-message')
            .addClass('text-danger')
            .text(_t('invalid_setting', 'Setting contains invalid JSON.'));
          $(this).find('.we-custom-field-setting').trigger('focus');
          return;
        }
        pendingBlockOptions.customFields.push({
      ...
      if (widgetId === 'weather') {
        collected.provider =
          $('#we-cfg-weather-provider').val() || 'openweather';
        ...
        if (valid && !calendarSources.length) {
          $('.we-cfg-message')
            .addClass('text-danger')
            .text(_t('calendar_needs_source', 'Add at least one calendar.'));
          valid = false;
        }
        if (valid) {
        ...
        if (!iframeUrl) {
          $('.we-cfg-message')
            ...
            .text(
              _t(
                'invalid_iframe_url',
                'Enter a valid URL for the iframe (e.g. http://192.168.1.x:8080).'
              )
            );
          ...
          widgetConfigs.iframe = {
            frameurl: iframeUrl,
            height: $.trim(
              $cfgModal.find('[data-cfg-key="iframe_height"]').val() || ''
            ),
            scrollbars: $cfgModal
              .find('[data-cfg-key="iframe_scrollbars"]')
              .is(':checked')
              ? 1
              : 0,
            scaletofit: $.trim(
              $cfgModal.find('[data-cfg-key="iframe_scaletofit"]').val() || ''
            ),
            aspectratio: $.trim(
              $cfgModal.find('[data-cfg-key="iframe_aspectratio"]').val() || ''
            ),
            forcerefresh: $cfgModal
              .find('[data-cfg-key="iframe_forcerefresh"]')
              .is(':checked')
              ? 1
              : 0,
            refresh:
              $.trim(
                $cfgModal.find('[data-cfg-key="iframe_refresh"]').val() || ''
              ) || '300',
          };
        }
        ...
        if (!xmltvUrl) {
          $('.we-cfg-message')
            ...
            .text(
              _t(
                'invalid_xmltv_url',
                'Enter a valid URL for the XMLTV TV Guide (e.g. http://my-epg-server/guide.xml).'
              )
            );
          ...
          widgetConfigs.xmltvguide = {
            xmltvurl: xmltvUrl,
            channels: $.trim(
              $cfgModal.find('[data-cfg-key="xmltv_channels"]').val() || ''
            ),
            maxitems:
              $.trim(
                $cfgModal.find('[data-cfg-key="xmltv_maxitems"]').val() || ''
              ) || '10',
            layout:
              $.trim(
                $cfgModal.find('[data-cfg-key="xmltv_layout"]').val() || ''
              ) || '0',
            separator:
              $.trim(
                $cfgModal.find('[data-cfg-key="xmltv_separator"]').val() || ''
              ) || '-',
            refresh:
              $.trim(
                $cfgModal.find('[data-cfg-key="xmltv_refresh"]').val() || ''
              ) || '3600',
          };
        }
        ...
        $cfgModal.find('.we-radio-row').each(function (index) {
          ...
          if (!stationFile || !/^https?:\/\/\S+$/i.test(stationFile)) {
            $('.we-cfg-message')
              ...
              .text(
                _t(
                  'invalid_radio_url_prefix',
                  'Enter a valid HTTP(S) stream URL for station'
                ) +
                  ' ' +
                  (index + 1) +
                  '.'
        ...
        if (valid && !tracks.length) {
          $('.we-cfg-message')
            .addClass('text-danger')
            .text(
              _t('invalid_radio_no_stations', 'Add at least one radio station.')
            );
          valid = false;
        }
        ...
        $cfgModal.find('.we-timegraph-value-row').each(function (index) {
          if (!valid) return;
          var rowValue = $.trim(
            $(this).find('.we-timegraph-value-value').val() || ''
          );
          var rowIdx = $.trim(
            $(this).find('.we-timegraph-value-idx').val() || ''
          );
          var rowLabel = $.trim(
            $(this).find('.we-timegraph-value-label').val() || ''
          );
          ...
          if (!rowValue) {
            $('.we-cfg-message')
              ...
              .text(
                _t(
                  'timegraph_value_required',
                  'Enter a value (e.g. Temp or NettUsage) for value'
                ) +
                  ' ' +
                  (index + 1) +
                  '.'
  ...
  function _collectConfigSettings() {
    ...
    if (widgetConfigs.xmltvguide) {
      ...
      configSettings.xmltv_separator =
        widgetConfigs.xmltvguide.separator || '-';
  ...
  function _buildWidgetPayloadEntry(item) {
    ...
    (blockOptions.customFields || []).forEach(function (row) {
      ...
      if (
        field === 'icon' ||
        field === 'c' ||
        _isProtectedCustomWidgetProperty(field)
      )
        return;
      var parsed =
    ...
    if (item.id === 'garbage') {
      ...
      entry.maxitems =
        parseInt(widgetConfigs.garbage.garbage_maxitems, 10) || 4;
    ...
    if (item.id === 'calendar') {
      ...
      entry.maxitems =
        parseInt(widgetConfigs.calendar.calendar_maxitems, 10) || 15;
    }
    if (item.id === 'clock') {
      ...
      if (clockType === 'stationclock') {
        ...
        entry.minutehandbehavior =
          ccfg.minutehandbehavior || 'BouncingMinuteHand';
        entry.secondhandbehavior =
          ccfg.secondhandbehavior || 'OverhastySecondHand';
      }
    }
    if (item.id === 'publictransport') {
    ...
    if (item.id === 'log') {
      ...
      if (
        typeof lgcfg.scrolltimeout !== 'undefined' &&
        lgcfg.scrolltimeout !== ''
      ) {
      ...
      if (lgcfg.height && lgcfg.height !== '')
        entry.logHeight = parseInt(lgcfg.height, 10) || 0;
    ...
    if (item.id === 'timegraph') {
      ...
      if (tgcfg2.duration && tgcfg2.duration !== '')
        entry.duration = parseInt(tgcfg2.duration, 10);
      if (tgcfg2.height && tgcfg2.height !== '')
        entry.timegraphHeight = String(tgcfg2.height);
      if (tgcfg2.xTicks && tgcfg2.xTicks !== '')
        entry.xTicks = parseInt(tgcfg2.xTicks, 10);
      if (tgcfg2.yTicks && tgcfg2.yTicks !== '')
        entry.yTicks = parseInt(tgcfg2.yTicks, 10);
      entry.xLabels = Number(tgcfg2.xLabels) !== 0;
      if (tgcfg2.animation && tgcfg2.animation !== '')
        entry.animation = parseInt(tgcfg2.animation, 10);
      if (
        tgcfg2.lineTension !== '' &&
        typeof tgcfg2.lineTension !== 'undefined'
      ) {
        entry.lineTension = parseFloat(tgcfg2.lineTension);
      }
      if (
        tgcfg2.pointRadius !== '' &&
        typeof tgcfg2.pointRadius !== 'undefined'
      ) {
  ...
  function _widgetEditorDraft(widgetId) {
    return {
      ...
      blockOptions: $.extend(
        true,
        {},
        widgetBlockOptions[widgetId] || _defaultWidgetBlockOptions()
      ),
    };
  }
  ...
  function _save() {
      ...
      (!_cameraWidgetConfig().cameras.length ||
        _cameraWidgetConfig().cameras.some(function (camera) {
          return (
            ...
            (camera.videoUrl && !/^https?:\/\/\S+$/i.test(camera.videoUrl))
          );
        }))
    ) {
      $('.we-message')
    ...
    if (selectedWidgets.iframe && !widgetConfigs.iframe.frameurl) {
      $('.we-message')
        ...
        .text(
          _t(
            'iframe_needs_url',
            'Enter a URL for the iFrame widget in its settings.'
          )
        );
      return;
    }
    ...
    if (selectedWidgets.xmltvguide && !widgetConfigs.xmltvguide.xmltvurl) {
      $('.we-message')
        ...
        .text(
          _t(
            'xmltvguide_needs_url',
            'Enter an XMLTV URL for the TV Guide widget in its settings.'
          )
        );
      return;
    }
          ...
          if (gridMode) {
            ...
            return _postWidgetData(
              'js/savegridlayout.php',
              {
                ...
                // Only pin these explicitly on the screen when they diverge
                // from the dashboard-wide default (see the matching comment
                // in layouteditor.js's _buildSavePayloads).
                pinGridColumns:
