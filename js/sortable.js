  ...
  $('.sortable')
    .sortable({
      ...
      receive: function (event, ui) {
        var html =
          '<div class="col-xs-4 col-icon"><em class="fas fa-question on icon"></em></div>';
...
function saveBlocks() {
  ...
  for (b in blocks) {
    conf += "\nblocks['" + b + "'] = {};\n";
    for (k in blocks[b]) {
      if (typeof blocks[b][k] === 'number')
        conf += "blocks['" + b + "']['" + k + "'] = " + blocks[b][k] + ';\n';
      else
        conf += "blocks['" + b + "']['" + k + "'] = '" + blocks[b][k] + "';\n";
    }
  }
  ...
  $('.sortable').each(function () {
    ...
    if (!curcol.hasClass('newblocks') && typeof done[key] === 'undefined') {
      ...
      curcol.find('> div').each(function () {
        ...
        if (
          typeof curdiv === 'number' ||
          (parseFloat(curdiv) &&
            curdiv.toLowerCase().indexOf('s') <= 0 &&
            curdiv.toLowerCase().indexOf('_') <= 0)
        ) {
          cols += curdiv + ', ';
      ...
      if (
        typeof columns[curcol.data('colindex')] !== 'undefined' &&
        typeof columns[curcol.data('colindex')]['width'] !== 'undefined'
      ) {
        conf +=
          '\ncolumns[' +
          curcol.data('colindex') +
          "]['width'] = " +
          columns[curcol.data('colindex')]['width'] +
          ';';
      }
      conf += '\n\n';
    }
  ...
  var html =
    '<div class="modal fade" id="blocksoutput" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">';
  html += '<div class="modal-dialog modal-dialog-settings">';
  html += '<div class="modal-content">';
  html += '<div class="modal-body" style="padding:20px;font-size:14px;"><br>';
  html +=
    '<strong>' +
    language.settings.blocksave +
