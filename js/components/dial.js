...
var DT_dial = {
  ...
  rotate: function(me){
    ...
    if (a >= -180 && a <= 60 || a >= 140 && a <= 180) {    /* within valid range */

      me.degrees = a >= 140 && a <= 180 ? a - 140 : a + 220;

      if (a >= -40 && a <= 60) {    /* right side, e.g. blue */

        $d.toggleClass("p180", true).removeClass("p0");                 
        $d.find(".bar").css({ webkitTransform: "rotate(180deg)" });
        $d.find(".fill").css({ webkitTransform: "rotate(" + me.degrees + "deg)" });

      } else if ((a >= 140 && a <= 180) || (a >= -180 && a <= -40)) {     /* left side, e.g. orange */

        $d.toggleClass("p0", true).removeClass("p180");    
        $d.find(".bar").css({ webkitTransform: "rotate(" + me.degrees + "deg)" });
      ...
      if (me.isSetpoint) {
        if (me.value >= me.setpoint) {    /* at or above setpoint */
      ...
      $(me.control).css({ webkitTransform: "rotate(" + (-140 + me.degrees) + "deg)" });
    }  
  },
  ...
  color: function (me) {
    if (isDefined(me.block.color)) {
      var c = $(me.mountPoint).css('color', me.block.color).css('color'); /* change all formats to rgb */
      me.color = c;
      me.rgba = c.split(',').length === 4
          ? c.replace(c.split(',')[3], '0.5)')                /* already rgba, make 50% opaque */
          : c.replace(')', ', 0.5)').replace('rgb', 'rgba');  /* convert rgb to rgba at 50% opaque*/
    } else {
  ...
  heating: function (me) {
    ...
    me.max = isDefined(settings["setpoint_max"]) ? settings["setpoint_max"] : 35;

    if(me.type === 'zone'){
      me.value = me.device.Temp;   
      me.setpoint = me.isSetpoint ? me.device.SetPoint : 20;
      me.boost = isDefined(settings["evohome_boost_zone"]) ? settings["evohome_boost_zone"] : 60;
