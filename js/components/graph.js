...
var DT_graph = {
    ...
    canHandle: function (block, key) {
            return block && block.devices || ((typeof key === 'string') && (key.substring(0,6) ==='graph_'))
    },
    run: function (me) {        

        if(typeof me.block !== 'undefined' && typeof me.block.devices !== 'undefined' || 
...
function getBlockConfig(me) {
    ...
    var graphDevices = [];
    var hasBlock = isDefined(me.block)? true : false;
    ...
    $.each(devices, function(i, idx) {   
        ...
        obj.primary = i === 0;
        obj.primaryIdx = obj.primary? obj.blockId + devices[i] : obj.blockId + devices[0];
...
function getGraphData(devices, selGraph){
    ...
    $.each(devices, function( i, device ) {	
        ...
        var graphIdx = device.hasBlock? device.blockId + device.idx : device.blockId;
        var graph = dtGraphs[graphIdx];
        var range = isDefined(selGraph)? selGraph : 'day';

        if(!graph.popup){
            if(graph.block.custom) range = graph.block.custom[Object.keys(graph.block.custom)[0]].range;
            if(graph.block.custom && selGraph) range = graph.block.custom[selGraph].range;            
        }
        ...
        if (dtGraphs[graph.primaryIdx].forced) {
            ...
            $.ajax({			
                url: settings['domoticz_ip'] + '/json.htm?username=' + usrEnc + '&password=' + pwdEnc + '&type=graph&sensor=' + dtGraphs[graphIdx].sensor + '&idx=' + dtGraphs[graphIdx].idx + '&range=' + range + '&method=1&time=' + new Date().getTime() + '&jsoncallback=?',
                type: 'GET',
                async: true,
                        ...
                        $.each(arrResults, function( z, d ) {
                            ...
                            if(graph.hasBlock && graph.block.graphTypes){
                                for (var key in d.result[0]) {
                                    if($.inArray(key, graph.block.graphTypes) !== -1 && key !== "d"){									
                                        arrYkeys.push(key);	
                                    }
                                }
                            } else {
                            ...
                            $.each(d.result, function( x, res ) {
                                var valid = false;
                                var interval = 1;
                                if(graph.hasBlock) interval = graph.range === 'last' || graph.range === 'month'? 1 : graph.block.interval;
                            ...
                            if(arrResults.length === counter){							
                                ...
                                dtGraphs[graph.primaryIdx].currentValues = currentValues;								
                                dtGraphs[graph.primaryIdx].data = multidata;
                                showGraph(dtGraphs[graph.primaryIdx], selGraph);                                                                
                            }						
                        });
                    }
                }				
    ...
    if (graph.forced || graph.popup) {
        ...
        if (graph.block.custom) {
            ...
            if (!graph.customRange) {
                console.log('custom graph, but graph selector ' + graph.range + ' not found')
            }
        }        
        createGraph(graph);
        //console.log(graph)
...
function createGraph(graph) {
    ...
    var title = '<div class="graphheader"><div class="graphtitle"><i class="fas fa-chart-bar" style="font-size:20px;margin-left:5px;color:' + buttonIcon + '">&nbsp;</i>' + graph.name + '';
    var span = '&nbsp;<span class="graphcurrent' + graphIdx + '">&nbsp;<i class="fas fa-equals" style="font-size:14px;color:' + buttonIcon + '">&nbsp;</i>&nbsp;';
    
    if(!graph.multigraph ){
        if(isDefined(graph.currentValue)) {
            title += span + graph.currentValue + ' ' + graph.txtUnit + '</span>';
        }
    } else {
        if (isDefined(graph.currentValues)) {
            title += span + graph.currentValues.join('<span style="color:' + graph.block.buttonsIcon + ';font-weight:900;font-size:16px;"> | </span>') + ' ' + graph.txtUnit + '</span>';
        }
    }
    title += '</div>';
    ...
    var mydiv = !graph.popup? $(graph.mountPoint + ' > div') : $(graph.mountPoint);
    ...
    var chartctx = document.getElementById('graphoutput_' + graphIdx ).getContext('2d');
    var graphProperties = getDefaultGraphProperties(graph);
    $.extend(true, graphProperties, graph.block);
    $.extend(graph, getGraphProperties(graph));
    ...
    if (graph.graphConfig) { //custom data
        ...
        if (!graph.block.graphTypes) {
            ...
            graph.ykeys = mySet.filter(onlyUnique);
            $.extend(graph, getGraphProperties(graph));
            
        } else {
            var newylabels = [];
            graph.block.graphTypes.forEach(function (element) {
                var idx = graph.ykeys.indexOf(element);
                if (idx >= 0) {
                    newylabels.push(graph.ylabels[idx]);
                }
            });
            if(!graph.multigraph) graph.ykeys = graph.block.graphTypes; //for backwards compatibility
            graph.ylabels = newylabels;
        }
    ...
    if(graph.multigraph && !graph.block.custom){
        
        var legendOrder	   = typeof graph.block.legend == 'object'? graph.legend : false;
        var drawOrderLast  = typeof graph.block.drawOrderLast == 'object'? graph.drawOrderLast : false;
        var drawOrderDay   = typeof graph.block.drawOrderDay == 'object'? graph.drawOrderDay : false;
        var drawOrderMonth = typeof graph.block.drawOrderMonth == 'object'? graph.drawOrderMonth : false;
        var order = false;
    ...
    var labelLeft = true
    var axisCount = graph.options && graph.options.scales && graph.options.scales.yAxes ? graph.options.scales.yAxes.length : 0;
    ...
    uniqueylabels.forEach(function(element, i){
        ...
        if (i < axisCount)
            $.extend(true, graphProperties.options.scales.yAxes[i], graph.options.scales.yAxes[i])
        labelLeft = !labelLeft;
    })
    ...
    graph.block.displayFormats? $.extend(graphProperties.options.scales.xAxes[0].time.displayFormats, graph.block.displayFormats) : graphProperties.options.scales.xAxes[0].time.displayFormats;
    graphProperties.options.scales.xAxes[0].ticks.maxTicksLimit = graph.maxTicksLimit;
...
function createButtons(graph, ranges, customRange) {	
    ...
    var btnTextList = {
        'last'	: btn.text && isDefined(btn.text[0])? btn.text[0] : language.graph.last_hours,
        'day'	: btn.text && isDefined(btn.text[1])? btn.text[1] : language.graph.today,
        'month'	: btn.text && isDefined(btn.text[2])? btn.text[2] : language.graph.last_month
    }
    ...
    if(isDefined(settings['graph_zoom']) && settings['graph_zoom'] === 1) {
        ...
        $(document).on("click", '#resetZoom' + graph.graphIdx, function() {
            Chart.helpers.each(Chart.instances, function(instance){
                if(instance.chart.canvas.id === $('#resetZoom' + graph.graphIdx).data('canvas')){
...
function updateGraphs(blockId, ids, range, popup){
    ...
    $.each(ids, function( i, idx ) {
        var graphIdx = blockId.substring(0,6) === 'graph_'? blockId : blockId + idx;
...
function getGraphProperties(graph) {

    var myProperties = dtGraphs[graph.graphIdx];
    var label = myProperties.txtUnit;
    var result = graph.data.result[0];
    var graphProperties = {};
    var arrYkeys = [];
    var arrYlabels = [];		
