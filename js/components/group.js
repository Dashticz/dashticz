  ...
  return {
    ...
    run: function (me) {
      ...
      if (me.block.devices || !me.block.idx) {
        ...
        if (typeof idx === 'string' && idx.toLowerCase()[0] === 's')
          idx = idx.substring(1);
        Domoticz.request('getscenedevices', false, { idx: idx })
          .then(function (res) {
            ...
            if (res && res.result) {
              ...
              throw new Error(
                '<br>Block ' + me.key + '<br>idx not valid: ' + idx
              );
          })
          .catch(function (err) {
  ...
  function groupClickHandler(me) {
    ...
    switch (toLower(me.block.switchMode)) {
      ...
      case 'toggleoff':
        ...
        break;
      default:
        newState = me.groupState === 'on' ? 'Off' : 'On';
    }
    me.devices.forEach(function (idx) {
      switchDevice(
        { idx: idx, type: 'group', device: Domoticz.getAllDevices()[idx] },
        newState
      );
    });
  }
