...
function loadGarbage() {
  ...
  var width = 12;
  if (
    typeof settings['garbage_width'] !== 'undefined' &&
    parseFloat(settings['garbage_width']) > 0
  )
    width = settings['garbage_width'];

  var html =
    '<div class="trash trash' +
    random +
    ' block_garbage col-xs-' +
    width +
    ' transbg" data-id="garbage">';
  if (
    typeof settings['garbage_hideicon'] !== 'undefined' &&
    parseFloat(settings['garbage_hideicon']) === 1
  ) {
    ...
    data = vevents
      .filter(function (vevent) {
        return moment(
          ...
          'YYYY-MM-DD'
        ).isBetween(date.start, date.end, null, '[]');
      })
      .map(function (vevent) {
        return {
          date: moment(
            ...
            element.pickupDates.forEach(function (dateElement) {
              ...
              dataFiltered.push({
                ...
                garbageType: mapGarbageType(
                  pickupTypes[element._pickupTypeText]
                ),
              });
            });
          });
...
function getWasteApi2Data(address, date, random, companyCode) {
  $.post(
    ...
    function (data) {
      $.post(
        'http://wasteapi2.2go-mobile.com/api/GetCalendar',
        {
          companyCode: companyCode,
          uniqueAddressID: data['dataList'][0]['UniqueId'],
          startDate: date.start.format('YYYY-MM-DD'),
...
function getMaxItems() {
  if (
    typeof settings['garbage_maxitems'] !== 'undefined' &&
...
function loadDataForService(service, random) {
  ...
  var serviceProperties = {
    ...
    best: {
      dataHandler: 'getIcalData',
      identifier:
        'https://www.gemeentebest.nl/afvalkalender/' +
        moment().format('YYYY') +
        '/' +
        address.zipcode +
        '-' +
    ...
    deurne: {
      dataHandler: 'getIcalData',
      identifier:
        'http://afvalkalender.deurne.nl/Afvalkalender/download_ical.php?p=' +
        address.zipcode +
        '&h=' +
        address.housenumber +
        '&t=&jaar=' +
