          ...
          curfull
            ...
            .html(
              '<strong>' +
                Math.round(temp) +
                _TEMP_SYMBOL +
                '</strong><span class="rainrate"></span>'
            );
...
//    _CORS_PATH +
    ...
    '?';
  if (isNumeric(settings['owm_city'])) site += 'id=' + settings['owm_city'];
  else site += 'q=' + settings['owm_city'] + ',' + settings['owm_country'];
...
function loadWeatherFull(location, country) {
  if (
    ...
    $('div.containsweatherfull').html(
      '<div class="weatherfull">' + containsweatherfull + '</div>'
    );
    ...
    $.getJSON(getOWMurl(true), function (currentforecast) {
      $('.containsweatherfull').each(function () {
        ...
        if (typeof currentforecast.list === 'undefined') {
          ...
          if (
            typeof settings['owm_days'] == 'undefined' ||
            settings['owm_days'] == '' ||
            settings['owm_days'] == 0
          ) {
            //torov5
            ...
            while (start < fcNumber) {
              ...
              if (
                ...
                if (settings['static_weathericons'] === 1)
                  html +=
                    '<div class="icon"><i class="wi ' +
                    wiclass +
                    '"></i></div>';
                else html += getSkycon(curfor.weather[0].icon, 'skycon');
                html +=
                  '<div class="day owmdescription">' +
                  Wdescription +
                  '</div><div class="temp"><span class="av_temp">' +
                  Math.round(temp) +
                  _TEMP_SYMBOL +
                  '</div>';
                if (settings['owm_min'] === 1)
                  html +=
                    '<div class="temp"><span class="nightT">' +
