...
function getEvohomeZoneBlock(block) {
  ...
  var faStatus = device.Status == "TemporaryOverride"
      ? "fas fa-stopwatch"
      : "far fa-calendar-alt";
  ...
  untilOrLastUpdate = device.Status == "PermanentOverride" 
      ? "Permanent" 
      : untilOrLastUpdate;
...
function getEvohomeControllerBlock(block) {    
    ...
    templateEngine.load("thermostat_evo_cont").then(function (template) {
      ...
      $div.find(".evoSelect").on("change", function () {
        ...
        $div.find(".input-status").text(language.evohome[newValue]).toggleClass("hide");
        $div.find(".evoSelect").toggleClass("hide");
      });
