...
function addCalendar(calobject, icsUrlorg) {
  if (!_PHP_INSTALLED) {
    ...
    infoMessage(
      ...
      'Calendar requires a PHP enabled web server</font>',
      0
    );
    return;
  }
  ...
  for (var c in icsUrl.calendars) {
    ...
    var color = '';
    if (typeof icsUrl.calendars[c].color !== 'undefined')
      color = icsUrl.calendars[c].color;
    ...
    $.getJSON(curUrl, function (data) {
      var url = this.url.replace(
        'https://cors-anywhere.herokuapp.com/http://ical-to-json.herokuapp.com/convert.json?url=',
        ''
      );
      ...
      for (var e in data) {
        ...
        var offset = 0;
        //debugger
        if (curCalendar.adjustTZ) offset = curCalendar.adjustTZ * 60 * 60;
      ...
      if (done == amountc) {
        var calFormat =
          typeof icsUrl.calFormat !== 'undefined' ? icsUrl.calFormat : 0;
...
function createCalendarModal(calobject, curUrl) {
  ...
  var html =
    '<div class="modal fade" id="calendar_' +
    random +
  ...
  for (var check in calitems) {
    ...
    for (var c in items) {
      ...
      if (counter <= calBlock.maxitems) {
        var styleColor =
          item1.color !== '' ? ' style="color:' + item1.color + '"' : '';
        ...
        if (item1.allDay == '') {
          ...
          if (!calBlock.startonly)
            widget +=
              '- ' +
              (startDayPart !== endDayPart ? endDayPart + ' ' : '') +
              endTimePart;
        } else {
...
function getCalendarFormatting(calBlock) {
  var calendarTimeFormat =
    typeof calBlock.timeFormat === 'undefined' ? 'LT' : calBlock.timeFormat; //The localized short time format will be used
  var calendarDateFormat =
    typeof calBlock.dateFormat === 'undefined' ? 'LD' : calBlock.dateFormat; // https://devhints.io/moment
...
function formatDate(caltime, dateFormat) {
  if (dateFormat === 'LD') {
    ...
    try {
      return tmpevent.toLocaleDateString(
        settings.calendarlanguage.replace(/_/g, '-'),
        dateOptions
      );
    } catch (err) {
  ...
  for (var check in calitems) {
    ...
    for (var c in items) {
      ...
      if (item1.allDay === false) {
        ...
        if (
          moment(enddate, test).format('YYYY-MM-DD') ===
          moment(startdate, test).format('YYYY-MM-DD')
        ) {
      ...
      if (calBlock.startonly) item1.enddate = '';
