...
- Device Config's Icon checkbox now writes the resolved static default icon
  ...
  nothing at all for icon - unlike hide_data/last_update/switch, which were
  already always written as an explicit true/false. `_resolveDefaultIcon()`
  (`js/deviceeditor.js`) reuses the same device-type lookup
  (`getBlockTypesBlock()`, `js/blocktypes.js`) a plain device tile's
  `handleDevice()` already renders with, so the icon actually in effect is
  now visible in the saved file. Deliberately only ever resolves a static
  `protoBlock.icon`, never a state-dependent `iconOn`/`iconOff` pair (e.g. a
  light bulb that toggles filled/outline with the device's on/off status):
  freezing a was-on/was-off snapshot into CONFIG.js would break that live
  toggling, and writing an empty string would look identical to Icon being
  off. Those devices keep resolving their icon at render time exactly as
  before.
- Fixed `type: 'dial'` blocks (including on grid screens) never showing an
  icon unless a custom icon value was typed in. `getColIcon()`
  (`js/dashticz.js`) paints a block's icon purely from `block.icon`, but the
  Device Editor's Icon checkbox only writes `block.icon` when a custom value
  was also typed (`js/deviceeditor.js` `_save()`) - every other widget falls
  back to its own default icon in that case (see the News/Sunrise fix in
  3.42.5), but `js/components/dial.js` had none, so a device that showed its
  normal type-based icon (`js/blocktypes.js`'s `getBlockTypesBlock()`, the
  same lookup a plain device tile's `handleDevice()` uses) lost it entirely
  the moment the Device Editor's Dial checkbox was checked. `dial.js`'s
  `defaultCfg` now reuses that same device-type lookup so a Dial-converted
  device keeps the icon it had before conversion; explicitly unchecking Icon
  (`block.icon === ''`) is left untouched.
- Fixed `type: 'dial'` blocks losing their icon and title in classic
  (non-grid, column-based) layouts, most visibly on devices converted via
  the Device Editor's Dial checkbox (#133), which previously rendered as a
  ...
  to pick the dial's circle diameter, but runs before the dial's own
  content is rendered - outside a grid item, `.dt_block` has no deliberate
  height at that point, just plain CSS auto-height driven by whatever
  little content exists so far (e.g. just the icon). Trusting that
  transient, near-empty height collapsed the whole tile - icon and title
  included - down to a sliver a few dozen pixels tall. Outside a grid item,
  the dial now sizes from its column width and any explicitly configured
  `height` only; a grid item's own height stays in the candidate list,
  since it's a deliberate constraint (row span, `overflow: auto`) and
  dropping it there could reintroduce the scrollbars this sizing logic was
